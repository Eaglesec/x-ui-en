...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/Eaglesec/x-ui-en/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_status() {
    ...
    case $? in
    0)
        echo -e "panel status: ${green}has been run${plain}"
        show_enable_status
        ;;
    1)
...
show_usage() {
    echo "How to use the x-ui-en management script: "
     echo "------------------------------------------"

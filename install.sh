...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://github.com/Eaglesec" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        echo -e "x-ui-en latest version detected：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/Eaglesec/x-ui-en/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Failed to download x-ui-en, please make sure your server can download Github files${plain}"
            exit 1
        fi
    else
        last_version=$1
        url="https://github.com/Eaglesec/x-ui-en/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
        echo -e "start installation x-ui-en v$1"
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Failed to download x-ui-en v$1, please make sure this version exists${plain}"
            exit 1
        fi
    fi
   ...
   #echo -e "If it is a fresh installation, the default web port is ${green}54321${plain}, and the default username and password are ${green}admin${plain}"
    ...
    echo -e "x-ui disable      - 取消 x-ui Auto-start"
    echo -e "x-ui log          - Check x-ui log"
    echo -e "x-ui v2-ui        - migrating this machine v2-ui account data to x-ui"
    echo -e "x-ui update       - renew x-ui panel"
